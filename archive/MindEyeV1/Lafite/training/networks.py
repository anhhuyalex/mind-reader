...
from Lafite.torch_utils import misc
from Lafite.torch_utils.ops import conv2d_resample, fma, bias_act, upfirdn2d
...
@torch_utils.misc.profiled_function
...
@torch_utils.persistence.persistent_class
class FullyConnectedLayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.activation == 'linear' and b is not None:
            ...
            x = x.matmul(w.t())
            x = torch_utils.ops.bias_act.bias_act(x, b, act=self.activation)
        return x
...
class Conv2dLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.register_buffer('resample_filter', torch_utils.ops.upfirdn2d.setup_filter(resample_filter))
        self.padding = kernel_size // 2
        self.weight_gain = 1 / np.sqrt(in_channels * (kernel_size ** 2))
        self.act_gain = torch_utils.ops.bias_act.activation_funcs[activation].def_gain
...
class MappingNetwork(torch.nn.Module):
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False):
        ...
        with torch.autograd.profiler.record_function('input'):
            if self.z_dim > 0:
                torch_utils.misc.assert_shape(z, [None, self.z_dim])
                x = normalize_2nd_moment(z.to(torch.float32))
            if self.c_dim > 0:
                torch_utils.misc.assert_shape(c, [None, self.c_dim])
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, fts, styles=None, return_styles=False, force_fp32=False, fused_modconv=None, **layer_kwargs):
        ...
        if img is not None:
            ...
            img = torch_utils.ops.upfirdn2d.upsample2d(img, self.resample_filter)
