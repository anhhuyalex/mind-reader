...
if utils.is_interactive():
    ...
    jupyter_args = f"--data_path=/fsx/proj-fmri/shared/mindeyev2_dataset \
                    --model_name={model_name} --no-use_prior \
                    --subj=1 --subj_list=1 --batch_size={batch_size} --num_sessions=3 \
                    --max_lr=3e-5 --mixup_pct=.33 --num_epochs=40 --no-ckpt_saving --stage2 \
                    --resume_from_ckpt=../train_logs/stage1_prior_subj01_h2048_3e5" #--resume_from_ckpt 
...
parser.add_argument(
    "--cache_dir",type=str,default='/fsx/proj-fmri/shared/cache',
)
parser.add_argument(
...
print("batch_size =", batch_size, "num_iterations_per_epoch =",num_iterations_per_epoch, "num_samples_per_epoch =",num_samples_per_epoch, flush=True)
...
for si, s in enumerate(subj_list):
    if num_sessions == -1:
        print(f"subj0{subj_list[si]} training with {nsessions_allsubj[s-1]} sessions")
        train_url = f"{data_path}/wds/subj0{s}/train/" + "{0.." + f"{nsessions_allsubj[s-1]-1}" + "}.tar"
    else:
        print(f"subj0{subj_list[si]} training with {num_sessions} sessions")
...
test_url = f"{data_path}/wds/subj0{subj_list[0]}/test/" + "0.tar"
print(test_url)
...
print(f"Loaded test dl for subj{subj_list[0]}! num_test={num_test[0]}\n")
...
if local_rank==0 and wandb_log: # only use main process for wandb logging
    ...
    if stage2:
        wandb_model_name = model_name + "_stage2"
    else:
...
if not stage2:
    ...
    assert resume_from_ckpt is not None or wandb_log is True
    if resume_from_ckpt:
