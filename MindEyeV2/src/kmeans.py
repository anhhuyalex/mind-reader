...
num_samples_per_epoch = 27000 // num_devices 
if args.filter_samples is not None:
    filter_samples_list = np.load(args.filter_samples) 
    num_samples_per_epoch = len(filter_samples_list) // num_devices 
    num_epochs *= (27000 / len(filter_samples_list))
    num_epochs = min(math.ceil(num_epochs), 120)
...
print("batch_size =", batch_size, "num_iterations_per_epoch =",num_iterations_per_epoch, "num_samples_per_epoch =",num_samples_per_epoch, "num_epochs =",num_epochs)
...
model.ridge = RidgeRegression(num_voxels_list, out_features=hidden_dim) #.to(data_type)
utils.count_params(model.ridge)
utils.count_params(model)

# test on subject 1 with fake data
# b = torch.randn((2,num_voxels_list[0])).to(torch.float32)
...
model.backbone = BrainNetwork(h=hidden_dim, in_dim=hidden_dim, clip_size=clip_emb_dim, out_dim=clip_emb_dim*clip_seq_dim) #.to(data_type)
...
if local_rank==0 and wandb_log: # only use main process for wandb logging
    ...
    #   "samples_list": np.array2string(filter_samples_list, separator=','),
      ...
      "num_samples": len(np.load(args.filter_samples)) if args.filter_samples is not None else None,
    }
    print("wandb_config:\n",wandb_config, args)   
...
print("model", model.ridge)
model, optimizer, *train_dls, lr_scheduler = accelerator.prepare(model, optimizer, *train_dls, lr_scheduler)
# optimizer, *train_dls, lr_scheduler = accelerator.prepare(optimizer, *train_dls, lr_scheduler)
print("model type", type (model))
print("model", model.ridge)
    ...
    for s, train_dl in enumerate(train_dls):
        with torch.cuda.amp.autocast(dtype=data_type):
            for iter, (behav0, past_behav0, future_behav0, old_behav0) in enumerate(train_dl):    
                ...
                voxel_iters[f"subj0{subj_list[s]}_iter{iter}"] = voxel0.to(data_type)
    ...
    if skip_train is False:
        for train_i in range(num_iterations_per_epoch): 
            with torch.cuda.amp.autocast(dtype=data_type):
                ...
                loss = loss_clip 
                print ("loss_clip", loss_clip.item(), flush=True)
    ...
    if local_rank==0:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=data_type): 
            for test_i, (behav, past_behav, future_behav, old_behav) in enumerate(test_dl):  
                ...
                voxel = test_voxel[test_indices].to(data_type).to(device)
                image = test_image[test_indices].to(data_type).to(device)
                print ("device", device)
