...
def get_parser_args():
    ...
    parser.add_argument(
        ...
        help="Number of training sessions to include (-1 = all possible sessions) (mutually exclusive with num_samples)",
    )
    parser.add_argument(
    ...
    # check that num_sessions and num_samples are mutually exclusive
    assert args.num_sessions != -1 or args.num_samples != -1, "must specify either num_sessions or num_samples"
...
def get_training_params(accelerator, args):
    ...
    args.batch_size = args.global_batch_size // args.num_devices
...
def load_ckpt(outdir, tag, model,   
        strict=True, stage2=False): 
    print(f"\n---loading {outdir}/{tag} ckpt---\n")

    # checkpoint = torch.load(f'{outdir}/{tag}')#, map_location="cpu")
    with open(f'{outdir}/{tag}', 'rb') as handle:
        checkpoint = utils.CPU_Unpickler(handle).load()
    if stage2: 
...
if __name__ == "__main__":
    ...
    accelerator = get_deepspeed_accelerator(args)
